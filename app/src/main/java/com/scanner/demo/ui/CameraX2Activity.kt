...
//            if (displayId == this@CameraX2Activity.displayId) {
...
//                        Intent(Camera.ACTION_NEW_PICTURE, Uri.fromFile(photoFile)))
            ...
            val intent = Intent(this@CameraX2Activity, MainActivity::class.java)
                            val bundle = Bundle()
                            bundle.putParcelable(ScanConstants.SELECTED_BITMAP, Uri.fromFile(photoFile))
...
//
//            Glide.with(this@CameraX2Activity)
//                    .load(photoFile)
...
//        }
    ...
    fun getNextDest(source : Int, dest : Int) : Int{
        var mean = (source+dest)/2
        if(abs(mean-dest) < 50){
            mean = dest
        }
        return mean
