...
class CircleImageView : ImageView {
   ...
   constructor(context: Context, attrs: AttributeSet, defStyle: Int) : super(context, attrs, defStyle){
    ...
    private fun calculateBounds(): RectF {
        val availableWidth = getWidth() - getPaddingLeft() - getPaddingRight()
        val availableHeight = getHeight() - getPaddingTop() - getPaddingBottom()

        val sideLength = Math.min(availableWidth, availableHeight)

        val left = getPaddingLeft() + (availableWidth - sideLength) / 2f
        val top = getPaddingTop() + (availableHeight - sideLength) / 2f
